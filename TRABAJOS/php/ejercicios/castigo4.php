<!DOCtype="number" html>
<html lang="es">
<head>
	<title>EJERCICIO ARRAY</title>
	<meta charset="utf-8">
	<link rel="stylesheet" type=
	"text/css" href="style.css">

</head>
<body>
	<nav class="inicio">
			<a href="inicio.html">INICIO</a>
			<a href="castigo1.php">ADICIONAL.1</a>
			<a href="castigo2.php">ADICIONAL.2</a>
			<a href="castigo3.php">ADICIONAL.3</a>
			<a href="castigo4.php">ADICIONAL.4</a>
			<a href="castigo5.php">ADICIONAL.5</a>
		</nav>
	<form action="castigo4.4.php" method="POST">
		
		
		<h2>EJERCICIO</h2>

		<div>
			<label>numero 1 fila 1</label>
			<input type="number" name="n1">
		<div>
			<label> numero 2 fila 1</label>
			<input type="number" name="n2">
		</div>
		<div>
			<label> numero 3 fila 1</label>
			<input type="number" name="n3">
		</div>
		<div>
			<label> numero 4 fila 1</label>
			<input type="number" name="n4">
		</div>
		<div>
			<label> numero 5 fila 1</label>
			<input type="number" name="n5">
		</div>

		<div>
			<label> numero 1 fila 2</label>
			<input type="number" name="n6">
		</div>
		<div>
			<label>numero 2 fila 2</label>
			<input type="number" name="n7">

		</div>
		<div>
			<label> numero 3 fila 2</label>
			<input type="number" name="n8">
		</div>
		<div>
			<label> numero 4 fila 2</label>
			<input type="number" name="n9">
		</div>
		<div>
			<label> numero 5 fila 2</label>
			<input type="number" name="n10">
		</div>
		<div>
			<input type="submit" name="enviar">
		</div>
			


 
   </form>
   </body>
   </html>